...
function App() {
	...
	if (errconfig) {
		...
		return (
			<div>
				<h1 className="text-gray-200">An error has occured while loading Config</h1>
			</div>
		);
	} else if (!config) {
		return <Loading />;
	...
	if (errsteam) {
		...
		return (
			<div>
				<h1 className="text-gray-200">An error has occured while loading Steam Library</h1>
			</div>
		);
	} else if (!steam) {
		return <Loading />;
	...
	if (errepic) {
		...
		return (
			<div>
				<h1 className="text-gray-200">An error has occured while loading Epic Library</h1>
			</div>
		);
	} else if (!epic) {
		return <Loading />;
	...
	if (firstLoad === false) {
		setFirstLoad(true);
		var cacheLoaded = fetch('http://localhost:666/cache/load', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ steamgames: steam.response.games }),
		});
		if (!cacheLoaded) {
			return <Loading />;
		}
	...
	try {
		...
		console.error('Epic games not found');
	}
								...
								>
									{games.map((data, i) => {
										return (
											<div key={i}>
												<div
													className="hover:cursor-pointer banner-container"
													onClick={() => {
														setInfomenu(data.appid);
													}}
												>
													<img
														className="rounded-xl pointer-events-none block"
														src={`http://localhost:666/cache/banner/${data.appid}`}
														alt={data.appid}
													/>
